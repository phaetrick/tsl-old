...
public:
    ...
    void process(T *in, T *out, int size) {
...
public:
    roomreverb(TRACK *t) : reverbbase(t), _smalll(_sr), _mediuml(_sr), _largel(_sr), _smallr(_sr, 1.01),
                           _mediumr(_sr, 1.01), _larger(_sr, 1.01) {
        //_mode = &t->params[MODFMVOCWET];
        //_early = &t->params[MODFMVOCDRY];
        _oldearly = *_early;
        _oldmode = *_mode;
