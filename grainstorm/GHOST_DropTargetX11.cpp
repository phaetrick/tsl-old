...
int32_t GHOST_DropTargetX11::m_refCounter = 0;
...
void GHOST_DropTargetX11::Initialize()
{
  ...
  int32_t dndTypesCount = ARRAY_SIZE(m_dndMimeTypes);
  int32_t counter;
...
void *GHOST_DropTargetX11::getURIListGhostData(uchar *dropBuffer, int32_t dropBufferSize)
{
  GHOST_TStringArray *strArray = nullptr;
  int32_t totPaths = 0, curLength = 0;

  /* Count total number of file paths in buffer. */
  for (int32_t i = 0; i <= dropBufferSize; i++) {
...
void *GHOST_DropTargetX11::getGhostData(Atom dropType, uchar *dropBuffer, int32_t dropBufferSize)
{
  void *data = nullptr;
...
bool GHOST_DropTargetX11::GHOST_HandleClientMessage(XEvent *event)
{
  Atom dropType;
  uchar *dropBuffer;
  int32_t dropBufferSize, dropX, dropY;
