...
void CheckBoxView::render(void *context) {
    ...
    bool checked = ev.getCurrentValue(_STATE) == 1.0;
    ...
    textDisplayCenteredFixed(this, canvas, paint, font, checked ? reinterpret_cast<const char*>(ICON_MD_CHECK_BOX)
...
void CheckBox::render(void *context) {
    ...
    tv.fg = _STATE->parameters[id].flags & Param::MidiParam && _STATE->midilearning.load() ? skcol::midilearning
                                                                      : skcol::fg;
    tv.render(context);
...
/*
View *view_create(DATA *p, const char *name, int32_t prio, bool perm, int orientation,
					  double scalefactor, uint8_t aspect_ratio, int32_t alignment, void (*init)(View *view), void (*destroy)(View *view),
					  void (*callback)(View *view, AInputEvent *event), void (*render)(View *view, void *context), void (*flush)(View *view, uint32_t mode, Colour *colour));
*/

// View* textview_create(DATA *p, const char *text, float textpadding, bool frame, uint32_t bg, uint32_t fg, int32_t prio, bool perm, int orientation, double scalefactor, uint8_t aspect_ratio, int alignment);
    ...
    switch (action) {
        ...
        case ACTION_UP:
            if (event.pointer_id == pointerid) {
                ...
                tsl::parameters::Event ev;
                ev.setup(_STATE, tindex, id);
                auto current = ev.getCurrentValue(_STATE);
                auto des = current == 1.0 ? 0.0 : 1.0;
                ev.value = des;
...
void CheckBox::computeSize() {
    int32_t textsize = _STATE->textsize2;
    ...
    switch (orientation) {
        case HORIZONTAL:
            ...
            break;
        case VERTICAL:
            tv.width = static_cast<Layout*>(parent)->childs.size() > 1 ? bounds.width() : parent->width.load();
            tv.height = textsize;

            //if(parent->orientation == VERTICAL)
            checkbox.height =
                    parent->height - tv.height < _STATE->knob_height_Knob ? parent->height -
                                                                       tv.height
...
CheckBox::CheckBox(tsl::AppState* appState, int32_t _orientation, int _alignment, uint16_t _id, long _offset, int _multi)
        : View(appState,
        VIEW_COMPUTESIZE, VIEW_COMPUTESIZE, _alignment, 10, false, "Checkbox"),
          tv(appState, _STATE->parameters[_id].name,
             CENTER_ALIGN), checkbox(appState) {
...
CheckBoxWrapped::CheckBoxWrapped(tsl::AppState* appState, int32_t al, uint16_t _id, long offset, int multi) : CheckBox(appState,
        HORIZONTAL, al, _id, offset, multi) {
    alignment = WRAP;
    aspect_ratio = 0;
}
