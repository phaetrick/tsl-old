...
{
  std::unordered_map<uintptr_t, int32_t> mTouchIDMap;
  int32_t mNextTouchID;
}
...
- (void) onTouchEvent:(ETouchEvent) eventType withTouches:(NSSet*) touches withEvent:(UIEvent*) event
{
  ...
  {
    CGPoint pos = [pTouch locationInView:self];
    ...
    point.ms.L = true;
    {
      uintptr_t key = (uintptr_t)pTouch;
      if (eventType == ETouchEvent::Began && [touches containsObject:pTouch]) {
        mTouchIDMap[key] = mNextTouchID++;
        if (mNextTouchID >= 16) mNextTouchID = 0;
      }
      auto it = mTouchIDMap.find(key);
      point.ms.touchID = (it != mTouchIDMap.end()) ? it->second : 0;
      // Only erase the ID mapping for touches that are actually ending/cancelling,
      // not for all touches in allTouches (which includes still-active fingers).
      if ((eventType == ETouchEvent::Ended || eventType == ETouchEvent::Cancelled)
          && [touches containsObject:pTouch]) {
        mTouchIDMap.erase(key);
      }
    }
    ...
    point.dX = pos.x - posPrev.x;
    point.dY = pos.y - posPrev.y;
...
- (void) layoutSubviews
{
  ...
  if (mGraphics)
    mGraphics->DrawResizeExplicit((int)drawableSize.width, (int)drawableSize.height);
}
@end
