...
// Shared by every IGraphicsWin in the process. Hosts are not guaranteed to open and close all
// editors on the same thread, and the plain int could tear. Note the counter being atomic makes
// the refcount correct but does not make refcount-plus-(Un)RegisterClassW one atomic step; a
// fully race-free version needs a mutex spanning both.
...
LRESULT CALLBACK IGraphicsWin::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  // mClosingWindow: once CloseWindow() has started tearing down the drawing backend,
  // mPlugWnd is still set, so anything that pumps messages in between (the GL ICD inside
  // wglDeleteContext, a host nested loop, a cross-thread SendMessage) would otherwise
  // deliver a WM_PAINT/WM_TIMER and draw into an abandoned context.
  if (!pGraphics || hWnd != pGraphics->mPlugWnd || pGraphics->mClosingWindow)
  {
...
void IGraphicsWin::DestroyGLContext()
{
  ...
  wglDeleteContext(mHGLRC);
  mHGLRC = nullptr; // don't leave a dangling (potentially recycled) handle for ActivateGLContext
}
#endif
...
#ifdef IGRAPHICS_GL
  // Was nested inside the tooltip block above, so with tooltips disabled (the default) it never
  // ran and CreateGLContext()'s binding stayed current on the host UI thread for the whole
  // editor session. Every draw path binds via ScopedGLContext, so leave nothing bound here.
  wglMakeCurrent(NULL, NULL);
#endif
...
#ifdef IGRAPHICS_GL
    // OnViewDestroyed() releases GPU resources, which issues real glDelete* calls. GL object
    // names are per-context and our contexts are unshared, so doing that while a *different*
    // instance's context is current deletes that instance's objects instead of ours.
    // Bind our own context first - but only if it isn't already current, since re-binding an
    // already-current context here is what used to deadlock in some hosts. DestroyGLContext()
    // unbinds immediately afterwards, so there is nothing to restore.
...
void IGraphicsWin::StopVBlankThread()
{
  if (mVBlankThread != INVALID_HANDLE_VALUE)
  {
    ...
    if (::WaitForSingleObject(mVBlankThread, 10000) == WAIT_OBJECT_0)
    {
      ::CloseHandle(mVBlankThread); // was leaked on every window close
    }
    else
    {
      // The thread is still running and still writing mVBlankCount on an object the caller is
      // about to destroy. Nothing here can fix that: waiting forever hangs the host (the thread
      // can be parked indefinitely in D3DKMTWaitForVerticalBlankEvent when the display sleeps)
      // and TerminateThread would strand the graphics adapter. Deliberately leak the handle
      // rather than close one the live thread still owns, and make the situation visible.
      DBGMSG("VBlank thread failed to exit within 10s; leaking its handle\n");
    }
...
void IGraphicsWin::VBlankNotify()
{
  ...
  ::PostMessageW(mVBlankWindow, WM_VBLANK, mVBlankCount, 0);
    ...
    #include "glad.c"
    #include "nanovg.c"
  #elif defined IGRAPHICS_TSL
    #include "IGraphicsTSL.cpp"
    #ifdef IGRAPHICS_GL
      #include "glad.c"
    #endif
