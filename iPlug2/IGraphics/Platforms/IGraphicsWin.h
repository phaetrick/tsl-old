...
private:
  ...
  // Set for the duration of CloseWindow(). mPlugWnd is still valid while the drawing
  // backend is being torn down, so WndProc would happily dispatch a re-entrant WM_PAINT
  // into a destroyed GrDirectContext. This flag closes that window.
