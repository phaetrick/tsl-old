...
#ifndef NO_IGRAPHICS
#if defined(IGRAPHICS_NANOVG) + defined(IGRAPHICS_SKIA)  + defined(IGRAPHICS_TSL)!= 1
...
#pragma mark - Drawing API implementation
  virtual void DrawResizeExplicit(int w, int h) {}
...
public:
  ...
  virtual void OnMouseDown(const std::vector<IMouseInfo>& points);

  /** Called when the platform class sends mouse up events */
  virtual void OnMouseUp(const std::vector<IMouseInfo>& points);

  /** Called when the platform class sends drag events */
  virtual void OnMouseDrag(const std::vector<IMouseInfo>& points);
  
  /** Called when the platform class sends touch cancel events */
  virtual void OnTouchCancelled(const std::vector<IMouseInfo>& points);
  ...
  virtual bool OnMouseDblClick(float x, float y, const IMouseMod& mod);
   ...
   * @return /c true on handled */
  virtual bool OnMouseWheel(float x, float y, const IMouseMod& mod, float delta);
   ...
   * @param mod IMouseMod struct contain information about the modifiers held
   * @return \c true if handled*/
  virtual bool OnMouseOver(float x, float y, const IMouseMod& mod);
  ...
  virtual void OnSetCursor() { SetMouseCursor(mCursorType); }
