 ...
 * @param precision The display precision in digits*/
  ...
  /** @return true if a custom display function has been set */
  bool HasDisplayFunc() const { return mDisplayFunction != nullptr; }

  /** StringToValueFunc allows custom string-to-value conversion (the inverse of a
   * custom DisplayFunc), so hosts can convert typed display strings back to values.
   * Return true if the string was handled. */
  using StringToValueFunc = std::function<bool(const char* str, double& value)>;

  /** Set the function to translate a display string back to a parameter value
