...
- (BOOL) startAudio
{
  NSError* error = nil;
  if (![engine startAndReturnError:&error]) {
    NSLog(@"[TSL] startAudio failed: %@", error);
    return NO;
  }
  return YES;
}
...
- (void) setupSession
{
  ...
  if (!isInstrument())
  {
    AVAudioSessionRecordPermission status = [session recordPermission];
    ...
    else if (status == AVAudioSessionRecordPermissionUndetermined)
    {
      [session requestRecordPermission:^(BOOL granted) {
        NSLog(@"[TSL] Microphone permission: %@", granted ? @"granted" : @"denied");
      }];
    }
  }
}

- (void) showMicPermissionDeniedAlert
{
  UIAlertController* alert = [UIAlertController
    alertControllerWithTitle:@"Microphone Access Required"
    message:@"Grainstorm needs microphone access for live input granulation. Please enable it in Settings."
