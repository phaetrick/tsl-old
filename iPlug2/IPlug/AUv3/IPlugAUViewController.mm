...
- (void) viewWillAppear:(BOOL) animated
{
  ...
  if (self.audioUnit)
  {
    NSLog(@"[TSL] viewWillAppear frame=%.0fx%.0f scale=%.1f", self.view.frame.size.width, self.view.frame.size.height, [UIScreen mainScreen].scale);
    PLATFORM_VIEW* pv = [(IPLUG_AUAUDIOUNIT*) self.audioUnit openWindow:self.view];
    pv.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    NSLog(@"[TSL] plugView frame=%.0fx%.0f", pv.frame.size.width, pv.frame.size.height);
  }
}
...
- (void) viewWillAppear
{
  ...
  if (mPlugView)
  {
    ...
    [NSLayoutConstraint activateConstraints:@[
      ...
      [mPlugView.trailingAnchor constraintEqualToAnchor:self.view.trailingAnchor],
      [mPlugView.bottomAnchor constraintEqualToAnchor:self.view.bottomAnchor]
    ]];
#else
