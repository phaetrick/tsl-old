...
#define OFF_TEXT "off"
/** First entry in both audio device combos. Selecting it hands the choice back to
 * the OS: the app then follows the system default device as it changes. The
 * resolved device name is still what gets stored in the ini — this is a mode, not
 * a device. */
#define SYSTEM_DEFAULT_TEXT "System Default"
...
public:
  ...
  * @return The ID RTAudio has given the audio device if found */
  ...
  /** Re-resolves the OS default audio devices and restarts the stream, but only for
   * the directions the user has not explicitly chosen in Preferences.
   * Main thread only — never call this straight from a CoreAudio notification. */
  void OnSystemDefaultDeviceChanged();

  /** Starts/stops watching the OS default device. No-op off macOS. */
...
private:
  ...
  /** True while the device in use was inherited from the OS default rather than
   * picked deliberately in Preferences. Only these directions follow the OS when
   * the default changes — a device the user chose on purpose stays chosen.
   * Persisted, because "did the user pick this?" cannot be re-derived from a name. */
  bool mInDevIsDefault = true;
  bool mOutDevIsDefault = true;
  /** Cached alongside mTempState so Cancel restores these too — they are not part
   * of AppState, so the existing state rollback does not cover them. */
  bool mTempInDevIsDefault = true;
  bool mTempOutDevIsDefault = true;
  bool mDefaultDeviceListenerInstalled = false;
