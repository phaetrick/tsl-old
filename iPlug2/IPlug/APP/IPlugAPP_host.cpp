...
// kAudioObjectPropertyElementMain is macOS 12+; deployment targets here are lower.
...
#ifdef OS_MAC
static const AudioObjectPropertyAddress kDefaultDeviceProps[2] = {
...
static OSStatus DefaultDeviceListenerProc(AudioObjectID, UInt32, const AudioObjectPropertyAddress*, void* pCtx)
{
  auto* pHost = static_cast<IPlugAPPHost*>(pCtx);

  // This fires on a CoreAudio notification thread while the HAL holds its own
  // lock. Tearing down and reopening the stream from here deadlocks against
  // that lock, so hop to the main thread and do the work there.
  dispatch_async(dispatch_get_main_queue(), ^{
    pHost->OnSystemDefaultDeviceChanged();
  });
...
IPlugAPPHost::~IPlugAPPHost()
{
  ...
  // Before CloseAudio: a notification arriving mid-teardown would otherwise
  // dispatch onto a host that is already being destroyed.
...
bool IPlugAPPHost::Init()
{
  ...
  InstallDefaultDeviceListener(); // follow the OS default unless the user picked a device
...
bool IPlugAPPHost::InitState()
{
  ...
  if (stat(mINIPath.Get(), &st) == 0) // if directory exists
  {
    ...
    if (stat(mINIPath.Get(), &st) == 0) // if settings file exists read values into state
    {
      ...
      // Default 1 (follow the OS) for settings files written before these keys
      // existed: whether the stored name was chosen or merely inherited cannot
      // be recovered from the name, and following is the better guess for the
      // large majority who never open Preferences.
      mInDevIsDefault = GetPrivateProfileInt("audio", "indevisdefault", 1, mINIPath.Get()) != 0;
      mOutDevIsDefault = GetPrivateProfileInt("audio", "outdevisdefault", 1, mINIPath.Get()) != 0;
...
void IPlugAPPHost::UpdateINI()
{
  ...
  WritePrivateProfileString("audio", "indevisdefault", mInDevIsDefault ? "1" : "0", ini);
  WritePrivateProfileString("audio", "outdevisdefault", mOutDevIsDefault ? "1" : "0", ini);
...
void IPlugAPPHost::InstallDefaultDeviceListener()
{
#ifdef OS_MAC
  if (mDefaultDeviceListenerInstalled || mNoIO || IsScreenshotMode())
    return;

  for (const auto& prop : kDefaultDeviceProps)
    AudioObjectAddPropertyListener(kAudioObjectSystemObject, &prop, DefaultDeviceListenerProc, this);

  mDefaultDeviceListenerInstalled = true;
#endif
}

void IPlugAPPHost::RemoveDefaultDeviceListener()
{
#ifdef OS_MAC
  if (!mDefaultDeviceListenerInstalled)
    return;

  for (const auto& prop : kDefaultDeviceProps)
    AudioObjectRemovePropertyListener(kAudioObjectSystemObject, &prop, DefaultDeviceListenerProc, this);

  mDefaultDeviceListenerInstalled = false;
#endif
}

void IPlugAPPHost::OnSystemDefaultDeviceChanged()
{
  if (mExiting || mNoIO || IsScreenshotMode() || !mDAC)
    return;

  // Nothing to follow if both directions were chosen deliberately.
  if (!mInDevIsDefault && !mOutDevIsDefault)
    return;

  ProbeAudioIO(); // refreshes mDefaultInputDev / mDefaultOutputDev

  bool changed = false;

  if (mOutDevIsDefault && mDefaultOutputDev)
  {
    auto name = GetAudioDeviceName(mDefaultOutputDev.value());
    if (name != mState.mAudioOutDev.Get())
    {
      mState.mAudioOutDev.Set(name.c_str());
  ...
  if (mInDevIsDefault && mDefaultInputDev)
  {
    auto name = GetAudioDeviceName(mDefaultInputDev.value());
    if (name != mState.mAudioInDev.Get())
    {
      mState.mAudioInDev.Set(name.c_str());
  ...
  // macOS sends this notification more than once per switch, and switching back
  // and forth quickly can queue several. Comparing names first makes the
  // duplicates free instead of restarting the stream repeatedly.
  if (!changed)
    return;

  DBGMSG("Default audio device changed, following to outdev = %s\n", mState.mAudioOutDev.Get());
...
bool IPlugAPPHost::TryToChangeAudio()
{
  ...
  if (!inputID)
  {
    if (mDefaultInputDev)
    {
      ...
      // Falling back means this device was not the user's choice, so it should
      // track the OS from here on.
      mInDevIsDefault = true;
...
void IPlugAPPHost::CloseAudio()
{
  ...
  mInputBufPtrs.Empty();   // <-- is this here?
  mOutputBufPtrs.Empty();  // <-- and this?
}
