...
void IPlugAPPHost::PopulateDriverSpecificControls(HWND hwndDlg)
{
  ...
  // Index 0 of both device combos is SYSTEM_DEFAULT_TEXT, so every real device
  // sits one place further down than its index in mAudioInputDevIDs /
  // mAudioOutputDevIDs. Leaving these at 0 selects "System Default", which is
  // what happens whenever the corresponding IsDefault flag is set.
  int indevidx = 0;
  int outdevidx = 0;
  ...
  SendDlgItemMessage(hwndDlg,IDC_COMBO_AUDIO_IN_DEV,CB_ADDSTRING,0,(LPARAM)SYSTEM_DEFAULT_TEXT);
  SendDlgItemMessage(hwndDlg,IDC_COMBO_AUDIO_OUT_DEV,CB_ADDSTRING,0,(LPARAM)SYSTEM_DEFAULT_TEXT);
  ...
  for (int i = 0; i<mAudioInputDevIDs.size(); i++)
  {
    ...
    if (!mInDevIsDefault && std::string_view(GetAudioDeviceName(mAudioInputDevIDs[i])) == mState.mAudioInDev.Get())
      indevidx = i + 1;
  }
  ...
  for (int i = 0; i<mAudioOutputDevIDs.size(); i++)
  {
    ...
    if (!mOutDevIsDefault && std::string_view(GetAudioDeviceName(mAudioOutputDevIDs[i])) == mState.mAudioOutDev.Get())
      outdevidx = i + 1;
  }
  ...
  // The channel and sample-rate lists describe a concrete device, so "System
  // Default" (index 0) has to be resolved to whatever the OS default currently
  // is. Note indevidx/outdevidx are 1-based over the ID vectors now, so
  // indexing them directly would also run one past the end when the last device
  // is the selected one.
  if (mAudioInputDevIDs.size())
  {
    const uint32_t id = (indevidx > 0) ? mAudioInputDevIDs[indevidx - 1]
                                       : (mDefaultInputDev ? mDefaultInputDev.value() : mAudioInputDevIDs[0]);
    inputDevInfo = mDAC->getDeviceInfo(id);
    PopulateAudioInputList(hwndDlg, &inputDevInfo);
  }

  if (mAudioOutputDevIDs.size())
  {
    const uint32_t id = (outdevidx > 0) ? mAudioOutputDevIDs[outdevidx - 1]
                                        : (mDefaultOutputDev ? mDefaultOutputDev.value() : mAudioOutputDevIDs[0]);
    outputDevInfo = mDAC->getDeviceInfo(id);
...
WDL_DLGRET IPlugAPPHost::PreferencesDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch(uMsg)
  {
    case WM_INITDIALOG:
      ...
      mTempState = mState;
      _this->mTempInDevIsDefault = _this->mInDevIsDefault;
      _this->mTempOutDevIsDefault = _this->mOutDevIsDefault;
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam))
      {
        ...
        case IDCANCEL:
          ...
          // Unconditionally, not inside the state-compare below: toggling between
          // "System Default" and the device that currently *is* the default leaves
          // the stored name identical, so the compare would miss it and the mode
          // would silently survive a Cancel.
          _this->mInDevIsDefault = _this->mTempInDevIsDefault;
          _this->mOutDevIsDefault = _this->mTempOutDevIsDefault;
        ...
        case IDC_COMBO_AUDIO_IN_DEV:
          if (HIWORD(wParam) == CBN_SELCHANGE)
          {
            ...
            if (idx == 0) // SYSTEM_DEFAULT_TEXT — hand the choice back to the OS
            {
              _this->mInDevIsDefault = true;
              if (_this->mDefaultInputDev)
                mState.mAudioInDev.Set(_this->GetAudioDeviceName(_this->mDefaultInputDev.value()).c_str());
            }
            else
            {
              // A deliberate choice: stop following the OS default for this
              // direction, otherwise the next system change would silently
              // override what was just picked here.
              _this->mInDevIsDefault = false;
        ...
        case IDC_COMBO_AUDIO_OUT_DEV:
          if (HIWORD(wParam) == CBN_SELCHANGE)
          {
            ...
            if (idx == 0) // SYSTEM_DEFAULT_TEXT — hand the choice back to the OS
            {
              _this->mOutDevIsDefault = true;
              if (_this->mDefaultOutputDev)
                mState.mAudioOutDev.Set(_this->GetAudioDeviceName(_this->mDefaultOutputDev.value()).c_str());
            }
            else
            {
              _this->mOutDevIsDefault = false;
