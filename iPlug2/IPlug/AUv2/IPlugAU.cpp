    ...
    case kAudioUnitProperty_ParameterInfo:               // 4,  listenable
    {
      ...
      *pDataSize = sizeof(AudioUnitParameterInfo);
      if (pData)
      {
        ...
        // Params with a custom display func can also provide strings via
        // kAudioUnitProperty_ParameterStringFromValue - advertise that, so hosts
        // show natural-unit strings instead of the raw value.
        if (pParam->NDisplayTexts() || pParam->HasDisplayFunc()) pInfo->flags |= kAudioUnitParameterFlag_ValuesHaveStrings;
