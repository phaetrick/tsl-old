...
if(NOT TARGET iPlug2::IGraphics::Skia)
  ...
  set(SKIA_PATH ${HOME_PATH}/programming/sources/libraries/skia/headers)
  ...
  elseif(CMAKE_SYSTEM_NAME STREQUAL "iOS")
    if(PLATFORM STREQUAL "SIMULATORARM64" OR PLATFORM STREQUAL "SIMULATOR64")
        set(SKIA_LIB_PATH ${HOME_PATH}/programming/sources/libraries/skia/ios_sim/lib/arm64)
    else()
        set(SKIA_LIB_PATH ${HOME_PATH}/programming/sources/libraries/skia/ios/lib/arm64)
    endif()
    target_link_libraries(iPlug2::IGraphics::Skia INTERFACE
  ...
  elseif(APPLE)
    set(SKIA_LIB_PATH ${HOME_PATH}/programming/sources/libraries/skia/mac/lib)
    target_link_libraries(iPlug2::IGraphics::Skia INTERFACE
      ${SKIA_LIB_PATH}/libskia.a
      ${SKIA_LIB_PATH}/libsvg.a
      ${SKIA_LIB_PATH}/libskshaper.a
...
# TSL drawing backend
if(NOT TARGET iPlug2::IGraphics::TSL)
  add_library(iPlug2::IGraphics::TSL INTERFACE IMPORTED)
  target_link_libraries(iPlug2::IGraphics::TSL INTERFACE iPlug2::IGraphics)
  target_compile_definitions(iPlug2::IGraphics::TSL INTERFACE IGRAPHICS_TSL)

  set(SKIA_PATH ${HOME_PATH}/programming/sources/libraries/skia/headers)
  target_include_directories(iPlug2::IGraphics::TSL INTERFACE
    ${SKIA_PATH}
    ${SKIA_PATH}/include
    ${SKIA_PATH}/include/core
    ${SKIA_PATH}/include/effects
  ...
  if(WIN32)
    set(SKIA_LIB_PATH ${HOME_PATH}/programming/sources/libraries/skia/win-x64/lib)
    target_link_libraries(iPlug2::IGraphics::TSL INTERFACE
      ${SKIA_LIB_PATH}/skia.lib
      ${SKIA_LIB_PATH}/svg.lib
    )
 elseif(CMAKE_SYSTEM_NAME STREQUAL "iOS")
...
# Skia with Metal backend (macOS and iOS)
if(APPLE OR IOS)
  if(NOT TARGET iPlug2::IGraphics::TSL::Metal)
    add_library(iPlug2::IGraphics::TSL::Metal INTERFACE IMPORTED)

    target_compile_definitions(iPlug2::IGraphics::TSL::Metal INTERFACE
      IGRAPHICS_TSL
      IGRAPHICS_METAL
    )

    target_link_libraries(iPlug2::IGraphics::TSL::Metal INTERFACE
      "-framework Metal"
      "-framework MetalKit"
      iPlug2::IGraphics::TSL
    )
  endif()
endif()

# Skia with GL3 backend
if(NOT TARGET iPlug2::IGraphics::TSL::GL3)
  add_library(iPlug2::IGraphics::TSL::GL3 INTERFACE IMPORTED)

  target_compile_definitions(iPlug2::IGraphics::TSL::GL3 INTERFACE
    IGRAPHICS_TSL
    IGRAPHICS_GL3
  )
target_include_directories(iPlug2::IGraphics::TSL::GL3 INTERFACE 
    ${IGRAPHICS_DEPS_DIR}/glad_GL3/include 
    ${IGRAPHICS_DEPS_DIR}/glad_GL3/src
  )
  if(APPLE)
    target_link_libraries(iPlug2::IGraphics::TSL::GL3 INTERFACE
      "-framework OpenGL"
    )
  endif()
...
# Skia with CPU backend (software rendering)
if(NOT TARGET iPlug2::IGraphics::TSL::CPU)
  add_library(iPlug2::IGraphics::TSL::CPU INTERFACE IMPORTED)

  target_compile_definitions(iPlug2::IGraphics::TSL::CPU INTERFACE
    IGRAPHICS_TSL
    IGRAPHICS_CPU
  )

  target_link_libraries(iPlug2::IGraphics::TSL::CPU INTERFACE
    iPlug2::IGraphics::TSL
  )
endif()
...
if(NOT DEFINED IGRAPHICS_LIB)
  ...
  elseif (IGRAPHICS_BACKEND STREQUAL "TSL")
    if(IGRAPHICS_RENDERER STREQUAL "CPU")
      set(IGRAPHICS_LIB iPlug2::IGraphics::TSL::CPU)
    elseif(IGRAPHICS_RENDERER STREQUAL "GL3")
      set(IGRAPHICS_LIB iPlug2::IGraphics::TSL::GL3)
    else()
      set(IGRAPHICS_LIB iPlug2::IGraphics::TSL::Metal)
    endif()
  else()
    # NanoVG
