...
namespace tsl {
    namespace midi {
        ...
        const std::map<std::string, int> midiNoteNames = {
            {"a0", 21}, {"as0", 22}, {"b0", 23},
            ...
            {"c8", 108}, {"cs8", 109}, {"d8", 110}, {"ds8", 111}, {"e8", 112},
            {"f8", 113}, {"fs8", 114}, {"g8", 115}, {"gs8", 116},
            {"a8", 117}, {"as8", 118}, {"b8", 119},
            {"c9", 120}, {"cs9", 121}, {"d9", 122}, {"ds9", 123}, {"e9", 124},
            {"f9", 125}, {"fs9", 126}, {"g9", 127},
        };

        namespace MidiNotes {

            float midiToFreq(int note) {
                return 27.5f * powf(2.f, (float)(note - 21) / 12.f);
            }

            int freqToMidi(float freq) {
                return (int)((12.0 / log(2.0)) * log((double)freq / 27.5));
            }

            int noteNameToMidi(const std::string& noteName) {
                auto it = midiNoteNames.find(noteName);
                return (it != midiNoteNames.end()) ? it->second : -1;
            }
            ...
            std::string midiToNoteName(int note) {
                for (const auto& kv : midiNoteNames)
                    if (kv.second == note) return kv.first;
