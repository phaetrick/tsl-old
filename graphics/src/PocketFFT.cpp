...
NOINLINE static void pass2b(size_t ido, size_t l1, const cmplx *cc,
                            cmplx *ch, const cmplx *wa) {
    const size_t cdim = 2;

    if (ido == 1)
        for (size_t k = 0; k < l1; ++k) PMC (CH(0, k, 0), CH(0, k, 1), CC(0, 0, k), CC(0, 1, k))
    else
...
NOINLINE static void pass4b(size_t ido, size_t l1, const cmplx *cc,
                            cmplx *ch, const cmplx *wa) {
    const size_t cdim = 4;
...
NOINLINE static int passg(size_t ido, size_t ip, size_t l1,
    ...
    for (size_t i = 1; i < ip; ++i)
        wal[i] = {csarr[i].r, sign * csarr[i].i};
    ...
    for (size_t j = 1, jc = ip - 1; j < ipph; ++j, --jc)
        for (size_t k = 0; k < l1; ++k)
            for (size_t i = 0; i < ido; ++i) PMC(CH(i, k, j), CH(i, k, jc), CC(i, j, k),
                                                 CC(i, jc, k))
    for (size_t k = 0; k < l1; ++k)
        for (size_t i = 0; i < ido; ++i) {
            cmplx tmp = CH(i, k, 0);
...
NOINLINE static rfftp_plan make_rfftp_plan(size_t length) {
    ...
    if (!plan) return NULL;
    plan->length = length;
    plan->ch = RALLOC(double, plan->length);
    if (!plan->ch) {
        DEALLOC(plan);
        return NULL;
    }
...
/* initialize b_k */
    plan->tmp = RALLOC(double, 4 * plan->n + 2 * plan->n2);
    if (!plan->tmp) {
        DEALLOC(plan->mem);
        DEALLOC(plan);
        return NULL;
...
/* initialize a_k and FFT it */
    ...
    if (cfftp_forward(plan->plan, akf, fct) != 0) { return -1; }
...
/* inverse FFT */
    if (cfftp_backward(plan->plan, akf, 1.) != 0) { return -1; }
...
static int rfftblue_backward(fftblue_plan plan, double c[], double fct) {
    ...
    if (fftblue_fft(plan, tmp, 1, fct) != 0) { return -1; }
