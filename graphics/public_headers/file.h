...
struct WAF {
    ...
    size_t (*write)(WAF *waf, void *pointer, size_t samples){};
    ...
    long tell() {
        return f == nullptr ? (channels == 1 ? buf[0].size() * 2 : (buf[0].size() + buf[1].size()) *2) : ftell(f);
    }
    std::vector<short> buf[2];
...
WAF *waf_open_fd_mp3(int fd, int sr, uint8_t channels, int qual = 2);

WAF *waf_open_fd_flac(int fd, int sr, uint8_t channels, int bps = 16);
...
public:
    ...
    int open(int sr, int fd, int recformat, int channels = 2) {
        _format = recformat;
        _channels = channels;
        if (recformat == 0) {
            if ((f = std::unique_ptr<WAF>(
                    waf_open_fd((int) fd, sr, (short) channels, 16, WAVE_FORMAT_PCM))) ==
                nullptr) {
                status = -1;
            }
        } else if (recformat == 10) {
            if ((f = std::unique_ptr<WAF>(
                    waf_open_fd_flac((int) fd, sr, (short) channels, 16))) ==
                nullptr) {
                status = -1;
            }
        } else if (recformat == 11) {
            if ((f = std::unique_ptr<WAF>(
                    waf_open_fd_flac((int) fd, sr, (short) channels, 24))) ==
                nullptr) {
                status = -1;
            }
        } else {
            if ((f = std::unique_ptr<WAF>(
                    waf_open_fd_mp3(fd, sr, channels,
                                    recformat))) ==
                nullptr) {
                status = -1;
            }
    ...
    template<typename T>
    int write(T in, int numFrames) {
        if (status != 0)
            return status;
        if (_format != 11) {
            recbuf.resize(numFrames * _channels * sizeof(int16_t));
            auto tmp = (int16_t *) recbuf.data();
            for (int i = 0; i < numFrames * _channels; i++) {
                tmp[i] = tsl::to16Bit(in[i]);
            }
        } else {
            recbuf.resize(numFrames * _channels * sizeof(int32_t));
            auto tmp = (int32_t *) recbuf.data();
            for (int i = 0; i < numFrames * _channels; i++) {
                tmp[i] = tsl::to24Bit(in[i]);
            }
        }
        if (f->write(f.get(), recbuf.data(), numFrames) != numFrames)status = -2;
        return status;
    }

    long offset() {
        return f != nullptr ? f->offset : 0;
    }

    static std::map<int, std::string> errorToString;
    int status{};
private :
    std::vector<unsigned char> recbuf;
    std::unique_ptr<WAF> f{};
    int _channels{}, _format{};
};
